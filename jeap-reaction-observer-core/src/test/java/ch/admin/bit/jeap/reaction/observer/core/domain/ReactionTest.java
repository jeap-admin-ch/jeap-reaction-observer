...
class ReactionTest {
    ...
    void idReturnsTriggerIdWhenActionIsNullWithValidTrigger() {
        ...
        Reaction reaction = new Reaction(trigger, List.of());
    ...
    void idReturnsActionIdWhenTriggerIsNullWithValidAction() {
        ...
        String expectedActionIdHash = DigestUtils.md5Hex(action.id().value());
        assertEquals("#" + expectedActionIdHash, reaction.id());
    }
    ...
    void idReturnsConcatenatedIdsWhenBothTriggerAndActionArePresentWithValidProps() {
        ...
        String expectedActionIdHash = DigestUtils.md5Hex(action.id().value());
        assertEquals(
                "event:fqn1:" + DigestUtils.md5Hex("key1=value1") + "#" + expectedActionIdHash,
                reaction.id()
        );
    }
    ...
    void idHandlesEmptyPropsGracefullyForTriggerAndAction() {
        ...
        Reaction reaction = new Reaction(trigger, List.of(action));
        String expectedActionIdHash = DigestUtils.md5Hex(action.id().value());
        assertEquals("event:fqn1" + "#" + expectedActionIdHash, reaction.id());
    }
